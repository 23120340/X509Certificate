...
client.py
---------
Phía Client — 5 bước xác thực chứng chỉ X.509 self-signed.
  ...
  Bước 4: Kiểm tra CRL (tải từ CRL Distribution Points, đối chiếu serial).
...
def verify_signature(cert):
    """Bước 1: Verify chữ ký self-signed bằng public key trong chính cert."""
    ...
    try:
        ...
        return True, "Chữ ký HỢP LỆ (verified bằng public key của cert)"
...
def check_crl(cert):
    """Bước 4: Tải CRL từ CRL Distribution Points và đối chiếu serial number."""
    try:
    ...
    for url in crl_urls:
        try:
            ...
            if entry is not None:
                ...
                return False, f"Chứng chỉ BỊ THU HỒI theo CRL (thu hồi lúc {rev_date})"
